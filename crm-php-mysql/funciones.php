...
function agregarCliente($nombre, $edad, $ciudad)
{
    $bd = obtenerBD();
    $fechaRegistro = date("Y-m-d");
    $sentencia = $bd->prepare("INSERT INTO clientes(nombre, edad, ciudad, fecha_registro) VALUES (?, ?, ? ,?)");
    return $sentencia->execute([$nombre, $edad, $ciudad, $fechaRegistro]);
}

function obtenerClientes()
{
    $bd = obtenerBD();
    $sentencia = $bd->query("SELECT id, nombre, edad, ciudad, fecha_registro FROM clientes");
...
{
    $bd = obtenerBD();
    $sentencia = $bd->prepare("SELECT id, nombre, edad, ciudad, fecha_registro FROM clientes WHERE nombre LIKE ?");
...
{
    $bd = obtenerBD();
    $sentencia = $bd->prepare("SELECT id, nombre, edad, ciudad, fecha_registro FROM clientes WHERE id = ?");
    $sentencia->execute([$id]);
    return $sentencia->fetchObject();
}
function actualizarCliente($nombre, $edad, $ciudad, $id)
{
    $bd = obtenerBD();
    $sentencia = $bd->prepare("UPDATE clientes SET nombre = ?, edad = ?, ciudad = ? WHERE id = ?");
    return $sentencia->execute([$nombre, $edad, $ciudad, $id]);
}
...
function obtenerClientesPorciudad()
{
    $bd = obtenerBD();
    $sentencia = $bd->query("SELECT ciudad, COUNT(*) AS conteo FROM clientes GROUP BY ciudad");
    return $sentencia->fetchAll();
}
